...
export async function createOrder(
  ...
  try {
      ...
      tags: [
        ...
        ...(orderData.shipping
          ? [
              [
                'shipping',
                `30406:${orderData.shipping.eventId}:${orderData.shipping.methodId}`
              ]
            ]
          : []),
    ...
    if (!ndk.signer) {
      ...
      return {
        success: false,
        message: '[createOrder.ts]: Failed: No signer available'
      }
    }
    ...
    return {
      success: false,
      message: '[createOrder.ts]: There was an issue creating the order'
    }
  }
}
